...
class DataPlots():
    ...
    def plotIndividualCosts(self, show_plot=False, save_dir=None, filename='plotIndividualCosts'):
        ...
        total_ax.legend()
        self.saveAndShow(fig, show_plot, save_dir, filename)
    ...
    def plotCostPercentages(self, show_plot=False, save_dir=None, filename='plotCostPercentages'):
        ...
        overlay_ax.fill_between(self.com_task_data.time, self.rh_goal_cost_normalized, self.energy_cost_normalized, color=self.energy_color.medium, alpha=0.3, label='self.energy_cost')
        overlay_ax.fill_between(self.com_task_data.time, self.com_goal_cost_normalized, self.rh_goal_cost_normalized, color=self.rh_color.dark, alpha=0.3, label='self.rh_goal_cost')
        overlay_ax.fill_between(self.com_task_data.time, self.rh_tracking_cost_normalized, self.com_goal_cost_normalized, color=self.com_color.dark, alpha=0.3, label='self.com_goal_cost')
        overlay_ax.fill_between(self.com_task_data.time, self.com_tracking_cost_normalized, self.rh_tracking_cost_normalized, color=self.rh_color.light, alpha=0.3, label='self.rh_tracking_cost')
        overlay_ax.fill_between(self.com_task_data.time, 0, self.com_tracking_cost_normalized, color=self.com_color.light, alpha=0.3, label='self.com_tracking_cost')
    ...
    def plotTotalCostPercentages(self, bar_ax=None, show_plot=False, save_dir=None, filename='plotTotalCostPercentages'):
        ...
        bar_ax.bar(0, self.energy_cost_normalized_sum, color=self.energy_color.medium, label='self.energy_cost')
        bar_ax.bar(0, self.rh_goal_cost_normalized_sum, color=self.rh_color.dark, label='self.rh_goal_cost')
        bar_ax.bar(0, self.com_goal_cost_normalized_sum, color=self.com_color.dark, label='self.com_goal_cost')
        bar_ax.bar(0, self.rh_tracking_cost_normalized_sum, color=self.rh_color.light, label='self.rh_tracking_cost')
        bar_ax.bar(0, self.com_tracking_cost_normalized_sum, color=self.com_color.light, label='self.com_tracking_cost')
    ...
    def saveAndShow(self, figure, show_plot=False, save_dir=None, filename=None):

        if show_plot:
            plt.show()

        if save_dir is not None:
            full_path = self.checkAndCreateDir(save_dir)
            if filename is None:
                filename = 'figure'
            filename += '.png'
    ...
    def checkAndCreateDir(self, dir_path):
